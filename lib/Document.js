...
const defineMethod = function(obj, name, fn) {
...
function Document(obj, isNew) {
  if (type(obj) !== 'object') {
    throw new Error(`Document must be an object (got "${obj}")`);
  }
  let delta = { $set: {}, $unset: {} };
  let observer;
  ...
  defineMethod(obj, 'set', function(path, value) {
    const res = _setter.call(obj, delta, path, value);
    if (observer) {
      observer.next({ path: path, value: value });
    }
    return res;
  });
  ...
  defineMethod(obj, '$observable', function() {
    return observable;
  });
