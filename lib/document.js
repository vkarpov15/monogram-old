...
function _getter(delta, path) {
  const res = _.get(this, path);
  if (type(res) === 'array' || type(res) === 'object') {
    defineMethod(res, 'get', (_path) => {
      return _getter.call(this, delta, path + '.' + _path);
    });
    defineMethod(res, 'set', (_path, value) => {
      return _setter.call(this, delta, path + '.' + _path, value, res);
    });
  }
  return res;
}
