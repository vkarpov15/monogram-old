...
function Document(obj, isNew) {
  var delta = { $set: _.cloneDeep(obj), $unset: {} };
  var state = {};
  var observer = _createObserver(obj, delta, state);
  var ignoredPaths = {};
  var ignoredPathFunctions = [];
  ...
  defineMethod(obj, '$ignorePath', function(path, shouldIgnore) {
    if (arguments.length === 1) {
      if (typeof path === 'function') {
        ignoredPathFunctions.push(path);
        return;
      }
      ...
      for (let i = 0; i < ignoredPathFunctions.length; ++i) {
        let res = ignoredPathFunctions[i](path);
        if (res === true) {
          return true;
        }
        if (res === false) {
