...
function ModelFactory(db, options) {
  ...
  var model = function(doc, isNew) {
    ...
    var save = function*() {
      if (doc.$isNew()) {
        debug(`${db.databaseName}.${options.collection}: new doc ${require('util').inspect(doc)}`);
        ...
        debug(`${db.databaseName}.${options.collection}: updating doc with id ${doc._id}, delta: ${delta}`);
