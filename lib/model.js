    ...
    doc.$ignore(function() {
      var save = function*() {
        if (doc.$isNew()) {
          ...
          if (!delta) {
            yield context.collection.insert({ _id: doc._id });
            return;
          }
          debug(`Inserting new doc ${require('util').inspect(doc)}`);

          const res = yield context.collection.update({ _id: doc._id }, delta,
            { upsert: true });
          ...
          debug(`Updating doc with id ${doc._id}, delta: ${delta}`);
