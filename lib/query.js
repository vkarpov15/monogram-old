...
functions.filters = function(fn) {
  visitFilters(this.s.filter, fn, this.s.schema);
};

function visitFilters(filters, fn, schema, path) {
  ...
  _.each(filters, function(value, key) {
    ...
    if (typeof key === 'number') {
      ...
      newPath = join(newPath, key);
    }
    fn(filters, key, schema && schema._paths[newPath]);
    visitFilters(value, fn, schema, newPath);
  });
}
